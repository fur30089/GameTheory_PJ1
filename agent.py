...
print (state[0:4],state[12:16],state[3::4],state[0::4])

state=[0]*16
empty = [pos for pos, tile in enumerate(state[12:16]) if not tile]
empty=[i+12 for i in empty]
print (empty)

empty = [pos for pos, tile in enumerate(state[0:4]) if not tile] 
print (empty)

empty = [pos for pos, tile in enumerate(state[3::4]) if not tile]
empty=[i*4+3 for i in empty]  
print (empty)
...
#state[i]*4+3
#state2=[i*4+3 for i in state]

#empty = [pos for pos, tile in enumerate(state) if not tile]
