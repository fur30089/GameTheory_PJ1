...
class episode:
    ...
    def BagRule(self):
       ...
       if self.step()<=8:
           if self.step()%3==1:
               if str(self.ep_moves[self.step()-1][0])[1]=='1':
                 a=0
               elif str(self.ep_moves[self.step()-1][0])[1]=='2':
                 b=0
               elif str(self.ep_moves[self.step()-1][0])[1]=='3':
                 c=0           
           elif self.step()%3==2:
               if str(self.ep_moves[self.step()-1][0])[1]!='1' and str(self.ep_moves[self.step()-2][0])[1]!='1':
                 b=0;c=0
               elif str(self.ep_moves[self.step()-1][0])[1]!='2' and str(self.ep_moves[self.step()-2][0])[1]!='2':
                 a=0;c=0
               elif str(self.ep_moves[self.step()-1][0])[1]!='3' and str(self.ep_moves[self.step()-2][0])[1]!='3':
                 a=0;b=0
           slideway=10
       else:
           if ((self.step()-10)/2)%3==1:
               if str(self.ep_moves[self.step()-2][0])[1]=='1':
                 a=0
               elif str(self.ep_moves[self.step()-2][0])[1]=='2':
                 b=0
               elif str(self.ep_moves[self.step()-2][0])[1]=='3':
                 c=0  
           elif ((self.step()-10)/2)%3==2:
               if str(self.ep_moves[self.step()-2][0])[1]!='1' and str(self.ep_moves[self.step()-4][0])[1]!='1':
                 b=0;c=0
               elif str(self.ep_moves[self.step()-2][0])[1]!='2' and str(self.ep_moves[self.step()-4][0])[1]!='2':
                 a=0;c=0
               elif str(self.ep_moves[self.step()-2][0])[1]!='3' and str(self.ep_moves[self.step()-4][0])[1]!='3':
                 a=0;b=0                 
           if str(self.ep_moves[self.step()-1][0])[1]=='U':
               slideway=0
           elif str(self.ep_moves[self.step()-1][0])[1]=='D':
               slideway=1
           elif str(self.ep_moves[self.step()-1][0])[1]=='L':
               slideway=2                 
           elif str(self.ep_moves[self.step()-1][0])[1]=='R':
               slideway=3                 
       '''
       if self.step()<=8:
           a=1;b=0;c=0
       ...
       '''    
       #print (self.step())  
       return a,b,c,slideway    

    def a(self):
        if self.step()>8:
            return str(self.ep_moves[self.step()-1][0])[1]
#print (str(self.ep_moves[0][0]))
       
#str(self.ep_moves[self.step()][0])[1]==1 
#str(moves[0][0])[1]==1  
        
    def last_turns(self, play, evil):
        return self.take_turns(evil, play)
    
    def step(self, who = -1):    #who = -1???
        size = len(self.ep_moves)
        if who == action.slide.type:
            return int((size -9 + 2) / 2)
        if who == action.place.type:
            return size - int((size -8 + 1) / 2)
        return size
    
    def time(self, who = -1):
        if self.ep_moves:
            if who == action.slide.type:
                return sum([mv[2] for mv in self.ep_moves[slice(9, self.step()+1, 2)]]) # action, reward, time usage
            if who == action.place.type:
                return sum([mv[2] for mv in self.ep_moves[slice(0, 9, 1)]]) + sum([mv[2] for mv in self.ep_moves[slice(10, self.step()+1, 2)]]) # action, reward, time usage
